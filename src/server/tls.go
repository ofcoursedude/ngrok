...
import (
	...
	"github.com/ofcoursedude/ngrok/server/assets"
)
...
func LoadTLSConfig(crtPath string, keyPath string) (tlsConfig *tls.Config, err error) {
	...
	if crt, err = fileOrAsset(crtPath, "static/tls/snakeoil.crt"); err != nil {
		return
	}

	if key, err = fileOrAsset(keyPath, "static/tls/snakeoil.key"); err != nil {
		return
	}
